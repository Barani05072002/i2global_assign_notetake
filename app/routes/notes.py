...
import models
from schemas.response_schema import NoteResponse, NoteCreate
...
def create_note(note: NoteCreate, user_id: uuid.UUID, db: Session = Depends(get_db)):
...
def update_note(note_id: uuid.UUID, note: NoteCreate, db: Session = Depends(get_db)):
    db_note = db.query(models.Note).filter(models.Note.note_id == str(note_id)).first()
    if not db_note:
        raise HTTPException(status_code=404, detail="Note not found")
